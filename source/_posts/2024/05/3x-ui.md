...
##### 2、若要对版本进行升级，可直接通过脚本选择【2】----->【更新面板】，
##### 3、要安装所需的版本，请将该版本添加到安装命令的末尾。 e.g., ver v2.3.1:
...
##### 2、创建好入站协议之后，默认只有一个客户端，可根据自己需求继续添加，并编辑客户端，选择【Flow流控】为xtls-rprx-vision-udp443，
##### 3、其他：流量限制，到期时间，客户TG的ID等选项根据自己需求填写，
##### 4、一定要放行端口之后，确保端口能够ping通，再导入软件，
##### 5、点击二维码或者复制链接导入到v2rayN等软件中进行测试。
